...
export const createMovie = (req, res) => {
    const { title, original_title, release_date, runtime_in_minutes, overview, budget, revenue, mpa_rating, collection, poster_url, backdrop_url } = req.body;
    if (!title) {
  ...
  `;
    ...
    db.run(sql, params, function (err) {
        ...
        db.get('SELECT * FROM Movies WHERE movie_id = ?', [newId], (getErr, movie) => {
            if (getErr) {
...
export const updateMovie = (req, res) => {
    ...
    db.get('SELECT * FROM Movies WHERE movie_id = ?', [id], (checkErr, existing) => {
        if (checkErr) {
    ...
    `;
        ...
        db.run(sql, params, (err) => {
            ...
            db.get('SELECT * FROM Movies WHERE movie_id = ?', [id], (getErr, movie) => {
                if (getErr) {
                    return res.status(500).json({ error: getErr.message });
                }
